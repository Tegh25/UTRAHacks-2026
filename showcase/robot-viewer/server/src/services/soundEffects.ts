...
export async function generateSoundEffect(prompt: string, duration: number = 2): Promise<Buffer | null> {
  const apiKey = process.env.ELEVENLABS_API_KEY;

  if (!apiKey) {
    console.log('[ElevenLabs Sound] No API key found, cannot generate sound');
    return null;
  }
  ...
  try {
    ...
    console.error('[ElevenLabs Sound] Error generating sound:', error);
    return null;
  }
}
